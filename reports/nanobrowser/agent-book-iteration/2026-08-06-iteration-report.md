...
> **PRE-HARDEN / HISTORICAL / NON-GATEABLE：** 本报告使用 2026-08-06 的旧 evaluator；新协议已证明其可被 prompt 泄漏、缺证据与占位 provenance 伪造 PASS。以下数字只是当时观察，不得作为当前发布证据或 Skill 晋升依据。
...
`2026-08-06-iter-formal-batch`：**历史 pre-harden 记录 10/10**；在当前 gate 下 non-gateable，不能沿用其 `verified_pass` / `false_complete=0` 声明。
