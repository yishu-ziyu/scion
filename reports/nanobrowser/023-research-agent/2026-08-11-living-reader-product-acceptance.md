...
系统级研究机制、证据计数、去重、候选完成拒绝、决策矩阵、飞书回读门、恢复策略、进度控制台和生产打包均已验证；真实任务也留下了可观察的持久进度。用户讨论门已达到 `81/80`，但正式交付仍缺少 4 个合格产品、恰好三个能力的真实证据矩阵与飞书双回读，因此不能标记完成。
...
7. 继续观察后，操作记录增至 `567`，证据空间增至 `151` 条。用户讨论原始记录 `96` 条，合格且去重后 `81/80`，用户研究阶段已达标；产品原始记录 `28` 个，合格且去重后仍为 `26/30`，2 个未计入。任务进入 `2/5` 产品研究阶段。
...
| G23-3 产品证据 | 至少 30 个实际打开研究的产品，重要候选含独立用户证据 | 真实侧栏读取正式计数；证据测试排除讨论帖冒充产品，并按产品身份去重 | 恢复后原始记录 `28` 个，合格值仍为 `26/30`，2 个未计入；没有最终候选的重要产品交叉证据导出 | **FAIL** |
...
| 去重后的证据空间导出 | 只有真实计数与最小 E2E，没有本任务完整 81+26 合格记录导出 | FAIL |
...
durable_progress_after_retry: PASS / qualified users 76 -> 81; raw products 26 -> 28
```
