...
系统级研究机制、证据计数、去重、候选完成拒绝、决策矩阵、飞书回读门、恢复策略、进度控制台和生产打包均已验证；真实任务也留下了可观察的持久进度。用户讨论门已达到 `81/80`，产品研究门已达到 `30/30`，但正式交付仍缺少恰好三个能力的真实证据矩阵与飞书双回读，因此不能标记完成。
...
7. 继续观察后，公共侧栏操作记录增至 `589`。用户讨论原始记录 `96` 条，合格且去重后 `81/80`；产品原始记录 `32` 个，合格且去重后 `30/30`，2 个未计入。用户与产品研究门均已达标，任务进入 `3/5` 交叉验证与决策阶段，当前最终能力仍为 `0/3`。
8. 后续观察到操作记录继续增至 `621`，任务保持 `running` 和“正常推进”，持续读取仓库协议、架构与用户证据。当前没有被接受的三个能力决策，也没有飞书双回读。因此 G23-4、G23-5、G23-6 仍未通过。
...
| G23-3 产品证据 | 至少 30 个实际打开研究的产品，重要候选含独立用户证据 | 真实侧栏读取正式计数；证据测试排除讨论帖冒充产品，并按产品身份去重 | 恢复后原始记录 `32` 个，合格值 `30/30`，2 个未计入；任务已进入交叉验证阶段 | **PASS（产品研究门）** |
...
| 去重后的证据空间导出 | 只有真实计数与最小 E2E，没有本任务完整 81+30 合格记录导出 | FAIL |
...
durable_progress_after_retry: PASS / qualified users 76 -> 81; qualified products 26 -> 30; raw products 26 -> 32
```

当前唯一完整完成路径：从正在运行的 checkpoint 完成交叉验证，记录仓库 HEAD 与能力地图，提交恰好三个能力的 2+1+1 七问矩阵，创建并重新打开飞书研究表与决策文档，保存完整证据导出和最终 receipt，再按 G23-1..9 复验。
