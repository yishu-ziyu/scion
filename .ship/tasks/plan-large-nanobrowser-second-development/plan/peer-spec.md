...
- `currentExecutor` and `currentPort` are global background state (`projects/nanobrowser/chrome-extension/src/background/index.ts:25-28`). `new_task` and follow-up await long-running execution inside the Port listener (`index.ts:105-145`), while pause/cancel mutate the same object (`index.ts:147-163`).
- The side panel sends a `tabId` (`projects/nanobrowser/pages/side-panel/src/SidePanel.tsx:573-630`), but background validation never switches BrowserContext to that tab before constructing Executor (`index.ts:113-121`). Task start must bind the declared tab explicitly.
...
- Executor creates all agents/actions internally (`projects/nanobrowser/chrome-extension/src/background/agent/executor.ts:41-97`), so TaskManager needs a small injected factory function and hooks; a factory class or general plugin system is unnecessary.
- Planner `done` directly becomes success (`executor.ts:116-128`, `163-190`). Navigator's `done` action ignores its `success` value when setting `isDone` (`projects/nanobrowser/chrome-extension/src/background/agent/actions/builder.ts:155-162`). Both are completion candidates only.
...
- Planner currently declares no completion criteria (`projects/nanobrowser/chrome-extension/src/background/agent/agents/planner.ts:21-47`). Its prompt also treats login/credentials as `done` (`prompts/templates/planner.ts:32-48`); Navigator suggests solving CAPTCHA and also exits through `done` for login (`prompts/templates/navigator.ts:62-69`, `121-124`). These must yield `waiting_user`.

### Action execution and safety

- Both live and history execution converge on `NavigatorAgent.doMultiAction()` (`projects/nanobrowser/chrome-extension/src/background/agent/agents/navigator.ts:195-205`, `498-544`). It directly invokes `Action.call()` and logs raw args (`navigator.ts:366-441`). This is the root seam for one dispatcher.
- `Action.call()` already performs Zod validation (`projects/nanobrowser/chrome-extension/src/background/agent/actions/builder.ts:44-72`). Split it into parse plus parsed execution so ActionDispatcher reuses, rather than duplicates, validation.
...
- BrowserContext already owns URL policy and tab identity (`projects/nanobrowser/chrome-extension/src/background/browser/context.ts:32-59`, `276-341`). Keep this authority; do not add another allow-list.

### Privacy and replay

- Raw replay has callers in side panel, background, Executor, Navigator, settings, and storage (`SidePanel.tsx:400-531`; `index.ts:235-259`; `executor.ts:228-235`, `368-440`; `navigator.ts:461-627`; `projects/nanobrowser/packages/storage/lib/chat/history.ts:32-49`, `228-250`). Removing only the background command leaves both a data leak and dead UI.
...
- `createStorage` already provides local get/set/subscribe (`projects/nanobrowser/packages/storage/lib/base/base.ts:59-156`). One background writer is enough; no repository interface or lock service is justified.
- Favorites already provide the single local catalog and stable CRUD surface (`projects/nanobrowser/packages/storage/lib/prompt/favorites.ts:23-57`, `62-205`; `SidePanel.tsx:731-810`). Extend entries to a prompt/Skill union; do not create a second catalog.
- `Page` has fixed Puppeteer evaluation methods (`projects/nanobrowser/chrome-extension/src/background/browser/page.ts:341-438`, `441-483`) but no HTML media control. Add fixed site-neutral observation/control there. Do not emit or execute generated JavaScript.
- The Chrome-extension package has Vitest and only module tests today (`projects/nanobrowser/chrome-extension/package.json:10-15`; `src/background/browser/__tests__/context.test.ts:1-43`). There is no implemented extension E2E harness. Deterministic journey coverage must therefore run through TaskManager with fake Executor/page adapters, followed by real Chrome owner acceptance.
...
- `projects/nanobrowser/packages/storage/lib/task/{types,runtime,index}.ts`
- `projects/nanobrowser/chrome-extension/src/background/task/manager.ts`
- `projects/nanobrowser/chrome-extension/src/background/task/action-dispatcher.ts`
- `projects/nanobrowser/chrome-extension/src/background/task/completion.ts`
- focused tests under `chrome-extension/src/background/task/__tests__/`
- `projects/nanobrowser/chrome-extension/src/background/agent/factory.ts`
