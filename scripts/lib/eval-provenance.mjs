...
export function projectPath(rel) {
  ...
  return PROJECT_PREFIX ? `${PROJECT_PREFIX}/${clean}` : clean;
}

export function isAttestedDistPath(filePath, runtimeFile) {
  const normalized = String(filePath || '').replaceAll('\\', '/');
  const suffix = `dist/${runtimeFile}`;
...
export function gitTreeEntries(scionRoot, gitSha, prefix = PROJECT_PREFIX) {
  ...
  const output = String(runGit(scionRoot, ['ls-tree', '-r', '--full-tree', '-z', gitSha, '--', pathspec]));
  return output
    .split('\0')
    .filter(Boolean)
    .map(record => {
...
export function evaluatorPrefixes({ runner = [], verifier = '', taskId = '', suiteFiles = [] } = {}) {
  const prefixes = new Set([
    projectPath('scripts/eval-matrix.mjs'),
    projectPath('scripts/eval-gate.mjs'),
    projectPath('scripts/lib/eval-gate.mjs'),
    projectPath('scripts/lib/eval-harness.mjs'),
    projectPath('scripts/lib/eval-provenance.mjs'),
    projectPath('chrome-extension/scripts/lib/eval-provider.mjs'),
    projectPath('chrome-extension/scripts/lib/eval-verification.mjs'),
    projectPath('chrome-extension/scripts/lib/eval-trace-evidence.mjs'),
  ]);
  for (const file of runner) prefixes.add(projectPath(String(file).replace(/^\/+/, '')));
  for (const file of suiteFiles) {
    prefixes.add(projectPath(`chrome-extension/${String(file).replace(/^\/+/, '')}`));
  }
  if (runner.some(file => String(file).includes('action-agent-e2e'))) {
    for (const fixture of ['form.html', 'products.html', 'media.html']) {
      prefixes.add(projectPath(`chrome-extension/test/fixtures/${fixture}`));
    }
  }
  if (runner.some(file => String(file).includes('r1-extract-e2e')) || verifier === 'products_extract') {
    prefixes.add(projectPath('chrome-extension/test/fixtures/products.html'));
  }
  if (runner.some(file => String(file).includes('eval-frontier-task')) || /^F\d+$/.test(taskId)) {
    prefixes.add(projectPath('chrome-extension/test/fixtures/frontier/'));
  }
...
export function taskDefinitionHashAtCommit(scionRoot, gitSha, taskId) {
  const matrix = gitTreeEntries(scionRoot, gitSha).find(
    entry => entry.path === projectPath('scripts/eval-matrix.mjs'),
  );
...
export async function distAttestation(scionRoot, projectRoot) {
  ...
  for (const runtimeFile of runtimeFiles) {
    if (!files.some(file => isAttestedDistPath(file.path, runtimeFile))) {
...
async function prepareTrustedCommitCheckout({ worktreeRoot }) {
  const projectRoot = PROJECT_PREFIX ? path.join(worktreeRoot, PROJECT_PREFIX) : worktreeRoot;
