...
const OPERATION_INTENT =
  /(?:打开|访问|进入|跳转|点击|点一下|填写|填入|提交|登录|登陆|注册|下单|购买|翻页|滚动|截屏|截图|下载|刷新|关闭标签|新建标签)|\b(?:open|visit|go to|click|fill|submit|log ?in|sign ?(?:in|up)|scroll|screenshot|download|refresh|close|new tab)\b/i;
...
export function applyChatStreamDelta(messages: Message[], stream: ChatStreamState, delta: string): Message[] {
  ...
  if (index === -1) {
    return [...messages, { actor: 'system' as Message['actor'], content: delta, timestamp: stream.timestamp }];
  }
  const next = [...messages];
