...
describe('classifyAgentEvent', () => {
  ...
  it('does not put an acknowledgement into chat', () => {
    expect(
      classifyAgentEvent({
        actor: Actors.PLANNER,
        state: ExecutionState.STEP_START,
        details: '好的，我来读取当前页面并提取主题和引用。',
      }),
    ).toEqual({ action: 'progress' });
    expect(
      classifyAgentEvent({
...
describe('process noise in stored history', () => {
  it('hides acknowledgement-only assistant lines from chat', () => {
    const d = humanizeStoredMessage({
      actor: Actors.NAVIGATOR,
      content: '好的，我来读取当前页面内容并为你提炼主题与正文引用。',
      timestamp: 1,
    });
