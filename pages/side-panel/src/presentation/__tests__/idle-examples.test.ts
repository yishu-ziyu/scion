...
describe('idle home examples', () => {
  it('offers three concrete tasks that fill the composer', () => {
    ...
    expect(IDLE_EXAMPLES.map(item => item.id)).toEqual(['read-current', 'extract-list', 'inspect-form']);
    expect(IDLE_EXAMPLES.every(item => item.prompt.length >= 8)).toBe(true);
    expect(IDLE_EXAMPLES.every(item => item.prompt.includes('当前页'))).toBe(true);
  ...
  it('renders a hero, example rows, and no mode rail', () => {
    ...
    expect(html).toContain('读当前页');
    expect(html).toContain('抽出列表');
    expect(html).toContain('检查表单');
    expect(html).not.toContain('提交按钮');
