...
export function humanizeStoredMessage(msg: Message, copy: HumanCopy = DEFAULT_ZH_COPY): DisplayMessage {
  ...
  // Historical process telemetry / acknowledgements: hide from chat.
  if (
    isProcessNoiseContent(content) ||
    isProcessNoiseContent(embedded.body) ||
    isStatusOnlyAnswer(content) ||
    isStatusOnlyAnswer(embedded.body)
  ) {
    return {
      kind: 'system_note',
      title: copy.assistant,
...
export function classifyAgentEvent(
  ...
  if (actor === Actors.SYSTEM) {
    ...
    if (state === ExecutionState.TASK_OK) {
      ...
      if (
        details &&
        !looksLikeMachineToken(details) &&
        !isProcessNoiseContent(details) &&
        !isStatusOnlyAnswer(details)
      ) {
        const text = details.length > 200 ? `${details.slice(0, 200)}…` : details;
        return { action: 'append_assistant', content: text };
      }
      if (details && isStatusOnlyAnswer(details)) return { action: 'suppress' };
  ...
  if (actor === Actors.PLANNER || actor === Actors.NAVIGATOR || actor === Actors.VALIDATOR) {
    ...
    if (state === ExecutionState.STEP_OK) {
      if (!details || looksLikeMachineToken(details) || isProcessNoiseContent(details) || isStatusOnlyAnswer(details)) {
