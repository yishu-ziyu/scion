...
description: "在可验证闸门上多次 rollout，用奖励筛轨迹并沉淀 Skill；不训权重。Runner 已跑；候选已落盘。"
category: "product"
number: "006"
status: current
services: ["projects/chijie-browser", "experiments/agent-core-bakeoff"]
related: ["product/019", "product/020", "product/021", "product/004", "design/002"]
last_modified: "2026-08-11"
---

# 006 — 外环 RL 最小方案

## 状态（Owner 裁定 + 2026-08-06 证据）

| 项 | 值 |
|----|-----|
| 文档角色 | **执行中**（Wave 5：runner 已跑，Skill candidates 已生成） |
| 是否默认执行 | **是**（本地 fixture / 安全公网轨迹；真实站仍需 Owner 登录） |
| 与当前里程碑 | 服务 `long_horizon_v1` 与 019 Wave 5；**不**把旧 M3 飞书/B 站升为北极星 |
| 证据目录 | `reports/nanobrowser/outer-rl/`（含 `skills/candidates/`） |

不要再写 `deferred_plan_only`：外环不是“只写方案未动”。
仍禁止：未授权真站登录态乱跑、训权重、用旗舰模型刷正式分。
...
| 和北极星什么关系 | 服务 021 长程任务成功率与可复用 Skill，不另起产品线 |
...
| 里程碑 | 本方案角色 |
|--------|------------|
| M1/M2 已完成（历史） | 阶段 A = 回归，防外环改策略把短任务 10/10 打穿 |
| **long_horizon_v1（当前）** | 主服务对象：长程轨迹 → Skill 候选 → 回归 |
| 旧 M3（Owner 登录） | 阶段 C 真站收益；未登录前只做 A/B；**不是**产品北极星 |
| Skill 沉淀 | 阶段 D = 候选筛选与换输入重跑 |
| 正式分纪律 | MiniMax-M3；旗舰 run 不进正式 verified 率 |

**不**把 `current_milestone` 改回旧 M3。本文件是 019 Wave 5 外环加速器，不是第二套产品北极星。
