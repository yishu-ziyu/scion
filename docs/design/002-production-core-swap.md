...
- **已落地：**
  ...
  - `agent/backends/observe-act-loop.ts` — 纯引擎 `runObserveActLoop`：observe → decide → act → re-observe（ticket 02 / S3）
  - `agent/backends/control-loop.ts` — 脚本化 control
  - `agent/backends/control-llm.ts` + `control-policy.ts` — 生产 LLM control（MiniMax JSON 硬化；调用 observe-act 引擎）
  - `agent/factory.ts` — 多后端；无脚本时走 LLM control
  - 单测：backends + observe-act-loop + control journey（批前 0 / 批后 1 / 回执）
- **残余：** 扩展内真实页 e2e 仍依赖 Owner 加载 unpacked；真实站 G3/G4 属 M3（Slice B/C）。
...
| 默认生产核（M2 目标） | `control`：observe→decide→act→re-observe（`runObserveActLoop`），经 `hooks.dispatchAction` 出副作用 |
...
6. **内容目标**：`isForbiddenTaskContentUrl` 拒绝把 `chrome-extension://` / `chrome://` 等当作任务内容页。
...
| Control 后端（脚本） | `chrome-extension/src/background/agent/backends/control-loop.ts` |
| Observe-act 纯引擎 | `chrome-extension/src/background/agent/backends/observe-act-loop.ts` |
| Control 后端（LLM） | `chrome-extension/src/background/agent/backends/control-llm.ts` |
