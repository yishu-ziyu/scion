...
export abstract class BaseAgent<T extends z.ZodType, M = unknown> {
  ...
  private setWithStructuredOutput(): boolean {
    ...
    // MiniMax M-series often emits <think>...</think> before JSON. LangChain
    // withStructuredOutput then fails with "Unexpected token '<'". Use manual
    // extraction + removeThinkTags instead (personal fork / MiniMax Token Plan).
    const name = (this.modelName || '').toLowerCase();
    const provider = (this.provider || '').toLowerCase();
    if (name.includes('minimax') || provider.includes('minimax')) {
      logger.debug(
        `[${this.modelName}] provider=${this.provider}: disable structured output, use manual JSON after stripping <think>`,
      );
      return false;
    }

    // custom_openai / any custom provider id often hosts reasoner models
    // (MiniMax Token Plan etc). Prefer manual JSON extraction for robustness.
    if (
      provider === ProviderTypeEnum.CustomOpenAI ||
    ...
    try {
      ...
      if (text) {
        ...
        try {
          ...
          // Surface validation details to the UI
        ...
        logger.warning(`[${this.modelName}] Empty model content in manual extraction mode`, {
          contentType: typeof response.content,
          keys: response && typeof response === 'object' ? Object.keys(response as object) : [],
        });
      }
    } catch (error) {
