...
**Single tree:** this directory is the only copy; `~/projects/yishu-browser` is a symlink here.
...
```bash
# From this directory (same as ~/projects/yishu-browser via symlink)
pnpm install
...
- Edit and build **here** (or via `~/projects/yishu-browser` symlink - same files)
...
| Build/load into Chrome today | **`~/projects/yishu-browser`** |
