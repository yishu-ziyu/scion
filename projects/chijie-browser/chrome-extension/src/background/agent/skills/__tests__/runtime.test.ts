...
describe('Skill discovery + runtime', () => {
  ...
  it('does not finish LH-04 after the IANA hop', async () => {
    ...
    expect(fromExample.handled).toBe(true);
    expect(fromExample.decision).toMatchObject({
      kind: 'action',
      name: 'go_to_url',
      criteria: [],
    });

    const fromIana = await runtime.tryDecide({
      roundId: 'r1',
      instruction,
      url: 'https://www.iana.org/help/example-domains',
      frame: frame('https://www.iana.org/help/example-domains'),
    });
    expect(fromIana.handled).toBe(true);
    expect(fromIana.decision).toMatchObject({
      kind: 'action',
      name: 'go_to_url',
      args: { url: 'https://en.wikipedia.org/wiki/Web_browser' },
      criteria: [],
    });
  });
