...
describe('022-SKILL-02 skill fallback', () => {
  it('failed matching skill returns handled=false with fallbackUsed and does not throw', async () => {
    ...
    expect(result.record?.skillId).toBe('test.always-fail');
    expect(result.record?.outcome).toBe('fail');
    expect(result.record?.fallbackUsed).toBe(true);
    expect(result.record?.candidateCount).toBeGreaterThanOrEqual(1);
  });
  ...
  it('throwing skill also falls back after recovery budget', async () => {
    ...
    expect(result.handled).toBe(false);
    expect(result.fallbackUsed).toBe(true);
    expect(result.record?.failureClass).toMatch(/skill_boom/);
  });
});
