...
**Scion repository override:** The root `AGENTS.md` controls the review procedure and final report. Run Standards and Spec as two independent passes, report them under separate `## Standards` and `## Spec` headings, and end each section with its own count and worst item. Do not merge or rerank findings across the two passes. Apply the five axes below within the Standards pass only where the project rules do not already delegate the check to lint or type checking.
...
This catches issues that a single model might miss because different models repeat different assumptions and failure patterns.
...
| "I wrote it, so I know it's correct" | Authors can miss their own assumptions. Every change benefits from an explicitly named second model review or human review. |
