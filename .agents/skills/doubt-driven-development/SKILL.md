...
A single-model reviewer can repeat assumptions from the original author; a model with a different architecture can catch different failure patterns. Doubt-driven is already opt-in for non-trivial decisions, so within that scope offering cross-model is part of the skill's value, not optional friction.
...
|---|---|
| "I'm confident, skip the doubt step" | Confidence correlates poorly with correctness on novel problems. Moments of certainty are exactly when unchecked assumptions persist. |
...
| "Cross-model is always better" | Cross-model can catch assumptions a single model repeats, but it adds cost and tool fragility. Offer it every interactive doubt cycle — the user decides whether the artifact warrants it. The agent's job is to surface the choice, not to gate it. |
