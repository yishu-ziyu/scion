...
Use Chrome DevTools MCP to let the agent inspect live browser state. This bridges the gap between static code analysis and live browser execution: the agent can inspect the rendered page, DOM, console logs, network requests, and performance data. Instead of guessing what's happening at runtime, verify it.
