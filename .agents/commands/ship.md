...
Read `.agents/skills/shipping-and-launch/SKILL.md` fully and follow it.
...
Spawn three Grok `spawn_subagent` children in a **single assistant turn** so they run in parallel (`background: true`). Sequential calls defeat the purpose of this command.

Each child uses `subagent_type: general-purpose` and `capability_mode: read-only`. Its prompt must start with "read this persona file fully and obey it", then the review target (staged changes or recent commits):

1. **code-reviewer** — `.agents/agents/code-reviewer.md` — five-axis review (correctness, readability, architecture, security, performance). Output the standard review template.
2. **security-auditor** — `.agents/agents/security-auditor.md` — vulnerability and threat-model pass. Check OWASP Top 10, secrets handling, auth/authz, dependency CVEs. Output the standard audit report.
3. **test-engineer** — `.agents/agents/test-engineer.md` — test coverage for the change. Identify gaps in happy path, edge cases, error paths, and concurrency scenarios. Output the standard coverage analysis.
...
4. **Accessibility** — Verify keyboard nav, screen reader support, contrast (not covered by the three personas — handle directly here, or invoke `.agents/references/accessibility-checklist.md`).
