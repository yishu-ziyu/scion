...
Spawn a Grok `spawn_subagent` (`subagent_type: general-purpose`, `capability_mode: read-only` unless live measurement needs execute). Its prompt must start with: read `.agents/agents/web-performance-auditor.md` fully and obey it. Also pass:
