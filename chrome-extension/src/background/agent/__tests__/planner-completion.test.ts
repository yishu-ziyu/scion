...
describe('Planner completion contract', () => {
  ...
  it('keeps login and CAPTCHA prompts out of automated success paths', () => {
    expect(plannerSystemPromptTemplate).toContain(
      'waiting_user.reason="login_required" ONLY when the page shows a real sign-in wall',
    );
    expect(plannerSystemPromptTemplate).toContain(
      'waiting_user.reason="captcha_required" ONLY when a CAPTCHA/challenge is visible',
    );
