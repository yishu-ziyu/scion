...
export const bilibiliOpenFirstVideoSkill: BrowserSkill = {
  ...
  async run(context): Promise<SkillResult> {
    ...
    const watchResult =
      judgeBilibiliWatchComplete(instruction, pageUrl, context.frame?.tab.title ?? '') ??
      (instructionRequestsFirstVideo(instruction) && /bilibili\.com\/video\/BV/i.test(pageUrl)
