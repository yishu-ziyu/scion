...
describe('Skill discovery + runtime', () => {
  ...
  it('does not preempt the loop by matching the user utterance', async () => {
    const registry = createSkillRegistry([themeCitationSkill, formFillSubmitSkill]);
    const instruction = '阅读当前页面，用一句中文概括核心主题，并引用一个正文中可见的细节；不要修改页面。';
    expect(
      discoverSkills({
        registry,
        instruction,
    ...
    const decided = await runtime.tryDecide({
      roundId: 'round-1',
      instruction,
      url: 'https://evermind.ai/zh/',
      observationText: 'EverMemOS 用于结构化长程推理',
      frame: frame('https://evermind.ai/zh/', 'EverMemOS'),
    });
    expect(decided.handled).toBe(false);
  ...
  it('does not finish LH-04 after the IANA hop', async () => {
    ...
    const fromExample = await runtime.tryDecide({
      ...
      frame: frame('https://example.com/'),
    });
    expect(fromExample.handled).toBe(false);
    expect(fromExample.fallbackUsed).toBe(true);
  });
