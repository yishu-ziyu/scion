window.buildDomTree = (
  ...
  function computeAccessibleName(node) {
    ...
    if (id && node.ownerDocument) {
      try {
        const lab = node.ownerDocument.querySelector(`label[for="${escapeIdForSelector(id)}"]`);
        const t = ((lab && (lab.innerText || lab.textContent)) || '').replace(/\s+/g, ' ').trim();
        if (t) return t;
      } catch {
        // invalid id
      }
  ...
  function enrichFormControlAttributes(node, nodeData) {
    ...
    const acc = computeAccessibleName(node);
    if (acc) nodeData.attributes.accname = acc.slice(0, 120);
  }
  ...
  function buildDomTree(node, parentIframe = null, isParentHighlighted = false, depth = 0) {
    ...
    if (
      ...
      enrichFormControlAttributes(node, nodeData);
    }
